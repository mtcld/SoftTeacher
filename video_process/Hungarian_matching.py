    ...
    def bipartite_matching(self, track_info, detect_info, pred_json, view_dict, view_id, file_name):
        ...
        for idx, dbox in enumerate(detect_boxes):
            ...
            for idy, tbox in enumerate(track_boxes):
                row_cost.append(
                    loss(detect_labels[idx], dbox, track_labels[idy], tbox))
        ...
        for r, c in zip(row_ind, col_ind):
            ...
            if cost_global[r, c] < 0.5:
                ...
                if cache_label not in self.cache.keys():
                    ...
                    print('debug hungarian : ', self.count_frame,
                          self.cache[cache_label][0])
                    if self.count_frame - self.cache[cache_label][0] == 1:
                        ...
                        self.cache[cache_label].insert(
                            1, self.cache[cache_label][1]+1)
                    else:
                        # self.cache[cache_label]=[self.count_frame,1]
                        self.cache[cache_label][0] = self.count_frame
                    ...
                    for i in range(1, self.threshold):
                        ...
                        for rc in self.history[curr_id]:
                            if view_dict[curr_id]['result'][0]['carpart']['labels'][rc['index_detect']] == track_labels[c]:
                                ...
                                view_dict[curr_id]['result'][0]['carpart']['labels'][rc['index_detect']
                                                                                     ] = detect_labels[r]
                                origin_id = rc['index_track']

                    # relabel the root of wrong tracking problem
                    view_dict[curr_id -
                              1]['result'][0]['carpart']['labels'][origin_id] = detect_labels[r]
                ...
                pred_json[0]['carpart']['labels'][detect_ind[r]
                                                  ] = track_labels[c]

                record = {'index_detect': detect_ind[r], 'detect': detect_labels[r],
                          'index_track': track_ind[c], 'track': track_labels[c]}
                ...
                print('relabel : ', detect_labels[r],
                      track_labels[c], cost_global[r][c])
            ...
            if cost_global[r, c] < 0.5:
                ...
                if cache_label not in self.cache.keys():
                    ...
                    if count_check == 1 or count_check == 0:
                        self.cache[cache_label] = [
                            self.count_frame, self.cache[cache_label][1]+1, self.cache[cache_label][2]+count_check]
                        # self.cache[cache_label][0] = self.count_frame
                ...
                if self.cache[cache_label][1] >= self.threshold:
                    ...
                    for i in range(1, self.threshold):
                        curr_id = view_id - i
                        print('trying to reverse .........', curr_id,
                              ' count check : ', self.cache[cache_label][2])
                        ...
                        if self.cache[cache_label][2] <= 1:
                            ...
                            for idrc, rc in enumerate(self.history[max_key]):
                                ...
                                print('len labels ', len(
                                    view_dict[curr_id]['result'][0]['carpart']['labels']))
                            ...
                            if len(clean_id) != 0:
                                self.history[max_key] = np.delete(
                                    np.array(self.history[max_key]), clean_id).tolist()
                            ...
                            if curr_id in self.history.keys():
                                ...
                                # np.delete()
                                if len(clean_id) != 0:
                                    self.history[curr_id] = np.delete(
                                        np.array(self.history[curr_id]), clean_id).tolist()
                                    check_del_cache = True

                    # relabel the root of wrong tracking problem
                    if origin_id != -1:
                        print('origin frame id ', view_dict[curr_id-1]['frame_id'], 'origin id : ', origin_id, ' / ', len(
                            view_dict[curr_id-1]['result'][0]['carpart']['labels']))
                        try:
                            view_dict[curr_id -
                                      1]['result'][0]['carpart']['labels'][origin_id] = detect_labels[r]
                        except:
                ...
                record = {'frame_id': frame_id,
                          'index_detect': detect_ind[r], 'detect': detect_labels[r], 'index_track': track_ind[c], 'track': track_labels[c]}
    ...
    def matching_damages(self, carpart_label, boxes1, ind_list1, coord_list1, info1, boxes2, ind_list2, coord_list2, info2):
        ...
        for r, c in zip(row_ind, col_ind):
            ...
            if cost_global[r, c] < 0.17 and \
                ...
                if abs(coord_list1[r] - coord_list2[c]) <= 2:
                    print('pair : ', r, c)
