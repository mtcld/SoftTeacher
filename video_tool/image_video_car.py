...
def main():
    ...
    #video_files=['video/video_2/FBAI_Car_05.MOV','video/IMG_5424.MOV','video/20220504_142734.mp4']
    # video_files = glob.glob('video/IMG_5424.MOV')
    #video_folders = ['video_out/IMG_5424_bin_5_first_frame']
    video_folders = [str(p) for p in Path('video_out').iterdir() if not p.is_file()]
    ...
    for folder in video_folders:
        ...
        name = os.path.basename(folder[:folder.find('_bin')])
        ...
        for f in files:
            ...
            # view_info = {'view': result[0]['carpart']['view'],'frame':frame,'result':result}
        ...
        with open(out_path+'/'+name+'_damges_by_frame_image_pipline.json', 'w', encoding='utf-8') as f:
            json.dump({'damaged_bin':damaged_by_bin,'result':final_result}, f, ensure_ascii=False, indent=4)
